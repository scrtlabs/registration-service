import {
    app,
    HttpResponseInit,
    InvocationContext,
    HttpRequest,
} from "@azure/functions";
import { fromBase64, SecretNetworkClient, TxResponse, Wallet } from "secretjs";
import { RaAuthenticate } from "secretjs/dist/tx/registration";

// *************** ENVIRONMENT VARIABLES  ********** //

const REGISTRATION_KEY: string =
    process.env["REGISTRATION_KEY"] ||
    "";

const MNEMONICS: string = process.env["MNEMONICS"] || "";
const SENDER_ADDRESS: string = process.env["SENDER_ADDRESS"] || "";

const SECRET_NODE_LCD: string = process.env["SECRET_NODE_LCD"] || "";
const CHAIN_ID: string = process.env["CHAIN_ID"] || "pulsar-3";

const GAS_FEE_IN_DENOM: number =
    Number(process.env["GAS_FEE_IN_DENOM"]) || 0.25;
const GAS_FOR_REGISTER: number =
    Number(process.env["GAS_FOR_REGISTER"]) || 150_000;
// *************** HELPER FUNCTIONS  ********** //

const faucet = {
    mnemonic: MNEMONICS,
    address: SENDER_ADDRESS,
};

async function createSendTx(
    client: SecretNetworkClient,
    certificate: Uint8Array,
): Promise<TxResponse> {
    let msg = new RaAuthenticate({ certificate, sender: client.address });
    return await client.tx.broadcast([msg], {
        gasLimit: GAS_FOR_REGISTER,
        gasPriceInFeeDenom: GAS_FEE_IN_DENOM,
    });
}

function initSecretClient(): SecretNetworkClient {
    let walletProto = new Wallet(faucet.mnemonic);

    return new SecretNetworkClient({
        url: SECRET_NODE_LCD,
        wallet: walletProto,
        walletAddress: faucet.address,
        chainId: CHAIN_ID,
    });
}

interface RegisterFormData {
    certificate: string;
}

// *************** MAIN ********** //

// the user calls this function and submits his claims (permit + address)
app.http("RegisterNode", {
    methods: ["POST"],
    authLevel: "anonymous",
    handler: async function (
        req: HttpRequest,
        context: InvocationContext,
    ): Promise<HttpResponseInit> {
        context.log("JavaScript HTTP trigger function processed a request.");

        let client = initSecretClient();

        const params: RegisterFormData = (await req.json()) as RegisterFormData;
        let cert: Uint8Array;
        try {
            cert = fromBase64(params.certificate);
        } catch (e) {
            context.log(`failed to decode base64 certificate: ${e}`);
            return {
                status: 400,
                jsonBody: {
                    status: "failed",
                    details: "certificate malformed",
                },
            };
        }

        let resp: TxResponse;
        try {
            resp = await createSendTx(client, cert);
        } catch (e) {
            context.log(`failed to send transaction: ${e}`);
            return {
                status: 500,
                jsonBody: { status: "failed" },
            };
        }

        if (resp.code) {
            context.log(`resp ${resp.code}: ${resp.rawLog}`);

            return {
                status: 500,
                jsonBody: { status: "failed", details: `${resp.rawLog}` },
            };
        }

        return {
            jsonBody: {
                status: "success",
                details: resp.jsonLog[0].events[0].attributes.find(
                    (attr) => attr.key === "encrypted_seed",
                ),
                registration_key: REGISTRATION_KEY,
            },
        };
    },
});
